...
**The tag names the challenge everywhere it leaves a mark.** The hidden challenged row carries it, the proven row carries it, the trail's messages contain it, and the ledger rows carry it in json, so from any one of them a person finds the rest by one string: which challenge proved this address, how many guesses it took, what the lambda sent and what the provider said. That is also what round robin will read, the seconds from send to proof by provider, and the natural place to record that number is a ledger row at the moment of proof, when enter holds the challenge's row_tick, its provider, and the trail's wrong-guess count in its hands; whether that row rides in this sprint or the ledger-adoption pass is an open choice below.

**One thing send keeps that the others gave up.** credentialOtpSend keeps its browserHash parameter, not to bind the challenge, which it never does, but for the ledger row it writes last: ledger_table's browser_hash margin is who and where, exactly what data-plan.md wants recorded there, and the endpoint has the hash from the door. Enter never had one, and gains one only if it writes the proof-time ledger row.
...
3. credentialOtpGet returns {addresses, challenges}, the list it returns today and the live challenges beside it. credentialOtpProven takes the tag and writes it into the proven row's json. Open: whether enter also writes a proof-time ledger row, Email. or Phone. with Proven., the provider, and json holding the tag, the seconds from send to proof, and the wrong-guess count, which would give enter a browserHash parameter for the ledger's margin; about eight lines, in this sprint or the ledger-adoption pass.
...
- The simulated inbox is a test seam and nothing more: an exported array in level3 that send fills only in simulation mode, holding the address, tag, and answer it would have sent. It is the honest replacement for reading the answer out of a note, and it lets the tests keep learning the code out of band, as a person does.
