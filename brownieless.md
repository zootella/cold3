...
- Every flow is coded for a person who already has a userTag. This sprint refactors the add-a-credential direction: the endpoint resolves the signed-in user from the Browser. row and hands the flow a userTag, and the flow never sees a browser. Sign-up is the other direction, where a stranger at a browser has no userTag yet, and it is a later sprint, previewed in first-night-accounts.md and framed in credential.md's userTag section. otp was coded first with sign-up in mind, which is why the browserHash reached its functions; that ambition is deliberately set aside here for consistency, and comes back as the sign-up sprint's job: mint the userTag at the first moment there is something to remember, then call these same flow functions with it. The flow functions don't change for sign-up; what changes is the step above them that answers who this is.
...
credential_table                                  # hidden by prove2 the moment the signature checks out, so the nonce works once
	...
	json:
		...
		connector: 'Injected.'                                          # new: how she connected, wagmi's injected connector for an extension or WalletConnect. for the relay to a phone app
...
credential_table
	...
	hash_text:  ''
	json:
		nonce:     'ygJTo9qkhSTeRoUQg44Kx'                               # new: which challenge proved it, so the proof points at its history
```

No trail messages. The envelope is gone, and prove1 returns the nonce alone. Of the envelope's five fields only the nonce needed a home: the address is the row's f triad, the action is the row's type and event_text, the expiration is row_tick plus twenty minutes, and the browserHash needs none, since the row belongs to its user and the signature is the proof of the wallet. The nonce is not a secret from anyone, since the page embeds it in the message the wallet displays before signing, so it rides json rather than the trail; a trail message would prove we issued it, but the row proves that already, and it would be a second read where the row is one. Two more things ride json that the proof doesn't need, because they tie the rows into a story: the challenge keeps which connector the page used, injected or WalletConnect, and the proof keeps the nonce of the challenge that proved it. From the proven row a person finds the hidden challenge by its nonce, and the two rows together say when the proof started, how she connected, and when it finished, which is the timing story by connector with no ledger row at all, since an ordinary proof involves no third party.

**prove1** takes the connector from the page beside the address, refuses any word but the two it knows, and writes the challenged row with the nonce and the connector in json, returning the nonce. **prove2**, in order:
...
4. **Spend the nonce.** Once the signature checks out, hide the challenge, by user, address, and nonce, so a captured signature replayed a minute later finds nothing. Then write the proof through credentialWalletSet, whose refusal rules run as today, with the nonce in the proof's json. Hiding before writing means a failure between the two leaves a spent nonce and no proof, and she starts over with a fresh one, which is the safe side to fail on.
...
how it went                  not recorded                                    the challenge keeps the connector and its row_tick, the proof keeps the nonce and its own
```
...
1. credentialWalletProve1 loses its browserHash parameter, takes the connector the page names, checks it against walletConstants.connectors, puts the nonce and the connector on the challenged row's json, and returns {nonce}.
2. credentialWalletProve2 loses its browserHash and envelope parameters, and runs the four steps above: parse and shape-check the nonce, find the challenge, validate and verify with the row's nonce, spend the nonce and write the proof, handing credentialWalletSet the nonce for the proof's json; a proof set directly, as the fixtures in the tests are, carries none.
3. The endpoint's two branches stop passing browserHash and the envelope, and the first passes the connector through; WalletPanel names the connector, Injected. or WalletConnect., from the button that connected, carries it into the prove flow, and stops destructuring an envelope from prove1's answer or passing one to prove2; credentialStore.walletProve1 gains the parameter and walletProve2 loses one. The `'ProveWallet.'` envelope action leaves with its last caller.
4. The grid tests, with these cases: the whole flow, the challenge carrying the connector, the proof carrying the nonce, and the challenge hidden beside the proof at the end; a signature over another address's challenge, and Carol at the same browser after Alice signs out, both hearing Expired.; the wrong wallet's signature hearing BadSignature., the right wallet over a nonce we never issued hearing Expired., and the real thing working; a replay after success hearing Expired., and a replay after a remove hearing Expired. too; a garbage message hearing BadSignature.; two tabs proving the same address, each with its own nonce, the second meeting WalletAlreadyProven.; a refused flow never minting a nonce; and a proof twenty minutes late, by ageNow, hearing Expired.
...
- The connector is context the page reports, and the one new value crossing the boundary in this move. prove1 accepts exactly the two words walletConstants lists and tosses on any other, the same loud answer the endpoint gives a malformed address, since our own page sends one of the two and anything else is a hand-made request. A tab loaded before the deploy that adds the field would send none and meet that toss once, until its next load, which is the stale-tab window every new required field has, and rare here because wallet proofs are rare.
...
*Send and record.* It calls the lambda, which hands the message to Twilio and returns the whole task. Then, in this order: it adds the note to the letter, first removing any earlier live note of hers to the same address; it writes the trail messages in one call; it writes the challenged row; and last it writes the ledger row holding the lambda's whole task, a record the hit-melt sprint added in September 2026, so the hosted ledger holds none yet, since the last code went out on August 19. The letter at the end of the request:
...
**What the page does with all this.** On every page render, on the server, credentialStore.load sends `Get.`, and attachState answers with the whole picture from credential_table: eight queries for a signed-in user, one for the Browser. row by hash and one per type by user, including the Email. and Phone. rows at every stage. From door.brownie it adds `task.otps`, the tag, start, and address of each live challenge with the answer left out; `task.enrollment` comes from credential_table since totp moved. But the server render cannot see localStorage, so the page's first paint never shows a live challenge; after hydration, app.vue's second onMounted asks the store whether the browser holds a brownie, and only then sends a second `Get.` carrying it, and OtpEnterList fills in a beat later. The brownie's last tenant: a letter today holds otp notes and nothing else.
...
credential_table                                  # hidden when the challenge closes: a right guess, the fourth wrong one, Held., or a resend replacing it
	user_tag:   '8quOfIYWkS1cmzj6nsgMm'
	type_text:  'Email.'
	event_text: 'Challenged.'
	f0_text:    'alice@example.com'
...
credential_table
	...
	hash_text:  ''
	json:
		tag:      'swrRT6UA0ZiywklAdfya7'                                # new: which challenge proved it, so the state points at the history
```

Trail messages at send, two where today there are two or three:

```
trail written
	'OTP opened challenge: address alice@example.com'      # the message the permit step counts, across every user who ever asked for a code to this address
	'OTP answer: tag swrRT6UA0ZiywklAdfya7 answer 4471'    # new: at enter the guess is hashed into this same message, and a row found means the guess is right
```

The brownie note and letter are gone, and so are the two trail messages that carried the challenge's state, opened-tag and closed-tag: the row is the state. A live challenge is a visible Challenged. row carrying a tag in json and under twenty minutes old, and a closed one is hidden. The challenged row is missing exactly two things the note has, the tag and the answer, and everything else the note carried is a copy of a column the row already has: its start is row_tick, its expiration row_tick plus twenty minutes, its owner user_tag, its type type_text, and its address the f triad. So the row gains the tag beside the provider, and the answer goes to the trail as a hash, where the check it needs is a yes or a no, in the same read that counts wrong guesses. Nothing binds the challenge to a browser: the row belongs to its user, and the code in the inbox is the proof.

**Send, after.** The four steps run as today with a different fifth: the challenged row carries the tag beside the provider in json, the trail gets the answer message beside opened-address, and any earlier live challenge of hers to the same address is hidden rather than closed on the trail. No note is added to anything, and the ledger row of the lambda's task is written last, as today, with the tag beside the task in its json. Challenged rows written before this sprint carry no tag, so they never read as live.
...
1. **Find the challenge**, this user's visible challenged row carrying this tag in json, through credential1 and a json-path filter on the handful of rows that come back. None, or one older than twenty minutes: Expired. Starting from the user is what makes a housemate's guess at her tag find nothing.
2. **Read the trail**, one call, two hashes where today there are three:

```
trail read, within twenty minutes
	'OTP guessed wrong: tag swrRT6UA0ZiywklAdfya7'         # must number fewer than four
	'OTP answer: tag swrRT6UA0ZiywklAdfya7 answer 4471'    # the guess hashed into the answer message; found means the guess is right
```

   Four wrong: Expired., a guard the hide at the fourth wrong guess already makes unreachable.
3. **Holder.** One credential_table read, as today: if another user proved this address while the challenge was live, hide the challenge and answer Held.
4. **Correct means the answer hash was found.** Right: credentialOtpProven as today, which reads for a holder again and for a visible challenged row of hers, then writes the proven row, now with the tag in its json; then hide the challenge, after the proof, since Proven wants to see the start it finishes. Wrong: write guessed-wrong, compute the lives left; none left means hide the challenge and Expired.; otherwise Wrong. with the count. No branch has a note to remove or a closing to record, because hiding is the closing.

**The snapshot.** task.otps keeps its shape, one entry per live challenge with tag, start, and address, and comes from the Email. and Phone. reads attachState already makes. credentialOtpGet widens the way credentialTotpGet did: beside the list of addresses with their highest event, it returns the list of live challenges, one per address where the newest visible challenged row carries a tag and is under twenty minutes old, with start from row_tick and the address rebuilt from the f triad. Same query, sifted twice. attachState joins the two types' challenges, sorted by start, and its letter parameter leaves with the last thing that read it. The enter boxes are on the page at first paint, and the mounted follow-up's second round trip has nothing left to fetch.

**The tag names the challenge everywhere it leaves a mark.** The hidden challenged row carries it, the proven row carries it, the trail's messages contain it, and the ledger rows carry it in json, so from any one of them a person finds the rest by one string: which challenge proved this address, how many guesses it took, what the lambda sent and what the provider said. That is also how round robin will read the seconds from send to proof by provider: the challenged row and the proven row carry the same tag and each carries its own row_tick, and the gap between them is the duration. No row ever records a duration; the tables hold granular facts, and deriving from them is server code's job. The ledger's part in the story, a row at the proof beside the row at the send, is a later sprint's.

**One thing send keeps that the others gave up.** credentialOtpSend keeps its browserHash parameter, not to bind the challenge, which it never does, but for the ledger row it writes last: ledger_table's browser_hash margin is who and where, exactly what data-plan.md wants recorded there, and the endpoint has the hash from the door. Enter never had one.

The audit of the change, property by property. Nothing gets weaker, one restriction is dropped on purpose, and one outcome changes its word.

```
the challenge is authentic    today: a note in a letter sealed under the envelope key    after: exists only as a row the server wrote
for this user                 note.userTag, matched by the flow                          user_tag, in the query
from this browser             the letter's browserHash, compared at the door             not checked: the row is not a bearer token, and the code in the inbox is the proof
fresh                         note.expiration, filtered at open                          row_tick plus twenty minutes
opened                        the opened-tag trail message                               the row exists
closed                        the closed-tag trail message                               the row is hidden
the answer                    note.answer, in the sealed letter                          a hash in the trail, matched against the hashed guess
wrong guesses                 counted in the trail                                       the same
codes to this address         counted in the trail                                       the same
the housemate at her box      SignedOut.                                                 Expired., since the lookup starts from his userTag and finds nothing of his
replay                        the trail refuses a replayed letter's spent guesses         nothing to replay: the count and the row are the state
the page is told              tag, start, and address per live challenge                 the same, on first paint
the page can send             any brownie; only ours opens                               a tag and a guess; only her own live challenge is found
```

### Testing and refactoring steps

The grid tests that walk this flow today are the otp suite near the top of grid.js — sanity, multiple addresses in one letter, expiry, three wrong then right and four wrong exhausting, a replacement code killing the previous, an attacker replaying the envelope getting no more guesses, the hard limit, the soft limit, code length by history, a correct guess closing on the trail, and "otp in the brownie" — and the "otp into credential" suite lower down: lifecycle rows for the signed-in user, a challenge belonging to who started it, a held address refusing everyone else, two users' challenges to one address coexisting with the enter-time claim check, and a removed address not resurrected by a late code. All of them keep proving the same rules against rows. Every test that read the answer out of a note reads it from a simulated inbox instead: in simulation mode send skips the lambda today, and after it also drops what it would have sent into an inbox the simulation database package carries, so a test learns the code the way a person does, by reading the message. "Multiple addresses in one letter" becomes two challenged rows; "two users' challenges coexist in one letter" becomes two rows; the replayed-envelope tests retire with their subject; "otp in the brownie" retires; "a correct guess closes on the trail" becomes a correct guess hiding the row; and the housemate's outcome in "a challenge belongs to the user who started it" changes from SignedOut. to Expired.

1. credentialOtpSend puts the tag beside the provider in the challenged row's json, writes opened-address and the answer to the trail, hides any earlier live challenge of hers to the same address in place of the closed message, fills the simulated inbox when it skips the lambda, puts the tag beside the lambda's task in the ledger row's json, and loses its letter parameter. It keeps browserHash for the ledger row.
2. credentialOtpEnter runs the four steps above and loses its letter parameter, every line that filtered notes, and the opened and closed hashes.
3. credentialOtpGet returns {addresses, challenges}, the list it returns today and the live challenges beside it. credentialOtpProven takes the tag and writes it into the proven row's json. No new ledger rows in this sprint; the ledger-adoption sprint after it adds the row at the proof.
4. attachState reads both fields from each of the two calls it already makes, builds task.otps from the challenges, and loses its letter parameter; the endpoint's `Get.`, `OtpSendTurnstile.`, and `OtpEnter.` branches stop passing door.brownie, and the empty-letter default at send leaves.
5. The page: OtpEnterList's comment says the enter boxes paint with the server render; credentialStore's otps comment says the answers live in the trail as hashes.
6. The grid tests, with these cases: the whole flow, a code sent and entered, the mention, challenge, and proof rows, and the challenge hidden at the end; email and phone in flight at once, two rows, each entered in turn; expiry by ageNow; three wrong then right, and four wrong exhausting with the row hidden; a resend hiding the earlier challenge, whose code then hears Expired.; the hard limit, the soft limit, and code length by history, unchanged but for the inbox; a right guess a second time hearing Expired.; a housemate's guess at her tag hearing Expired., with her challenge still live; a held address refusing everyone else at send and at enter; two users challenged at one address, the second hearing Held. after the first proves it; and a removed address taking its challenge with it, so a late correct code hears Expired. and no proof is saved. Every case reads rows back through credentialOtpGet, and through queryGet where the hide flag is the point.
7. Smoke, local, with the servers running here so their output lands in this session: send a code, see the enter box on first paint, refresh and see it still there, guess wrong and see the count fall, enter right and see the address proven; send to email and phone at once and enter both; send twice to one address and see the first code refused; sign in as the same user at a second browser and see the enter box there too.

**Status, September 7, 2026.** All three moves shipped: totp, wallet, and otp run on Challenged. rows, found by userTag, and the brownie carries nothing. The removal below is deferred, not begun. anonymous-users.md weighs where the state of a person with no account should live, and a sealed letter the browser holds is one of its three candidates, so the brownie stays in the code, empty and running, until that choice is made. If the choice goes to the database, as that document recommends, the removal runs as written here; if it goes to the browser, the brownie gets a new tenant and this section becomes its plan. Either way, nothing in it is load-bearing today: the door opens and reseals an empty letter on every request, the store's follow-up fires only for a browser still holding an old letter and fetches nothing new, and no code writes a note.
...
- The trail, for otp, after: three messages where today there are five. opened-address, one per send, counts codes to an address across every user who ever asked for one, for the hard limit, the soft limit, and the code length; it is the one protection in the flow that is about the address as a target rather than about a user, so it cannot live in any user's rows. guessed-wrong, one per wrong guess, counts toward four. The answer, one per send, is the yes-or-no check. The two that leave, opened-tag and closed-tag, carried state the row now holds. The trail's json column, documented as recoverable information beside the one-way hash proof, still has no production writer.
- The answer rides the trail rather than the row's json because hashes of messages held as proof is what the trail is, its column is named hash and sized for one, and every reader of it already knows so; credential_table's json has never held a hash, and a stranger reading the row would have to learn one more convention.
- A code behind a hash defends the casual view of credential_table — the dashboard scrolled, a coworker's phone camera, a screenshot in a support thread — and not the reader with the database and time, since a six-digit code with a known tag falls to enumeration. The defense is honest but partial while the ledger row of the send holds the lambda's task with the message text, code included, in the clear; that is data-cleanup.md's open item about the code in a ledger note, and this sprint neither fixes nor worsens it.
- The trail's replay defense goes quiet. Today the trail is a necessity because "an attacker could just replay the same valid brownie, guessing sequentially until they hit the correct answer"; a row cannot be replayed, because the row is the state.
- Outcomes and remedies: Held. closes a challenge another user won; Expired. covers a code past its time, the fourth wrong guess, a challenge replaced by a resend, a challenge already answered, an address removed mid-challenge, which takes its challenge with it, and a housemate guessing at someone else's box, which today hears SignedOut. That last case can only arise from a stale tab, since the snapshot never shows a user another user's boxes, and the page's Expired. handling clears the box on the next snapshot either way. SignedOut. stays the endpoint's answer for a request with no session at all.
- Send keeps its four steps and their order, and the claim step still writes the mention on every send, even one refused as Held., because repeated mentions of a held address are the evidence a confused user keeps typing an address that isn't theirs.
- The simulated inbox is a test seam and nothing more, and it lives outside level3: the database package grid.js builds and parks for simulation mode carries an inbox array beside pglite and clear, and send pushes into it through the getDatabase call it already makes, only when it skips the lambda. It holds the address, tag, and answer of each code that would have gone out. It is the honest replacement for reading the answer out of a note, and it lets the tests keep learning the code out of band, as a person does, with nothing test-shaped declared in level3.
