...
logContents.value = "version 2024mar2c";
...
function runTests() {
	...
	if (assertionsFailed || testsThrew) {
		...
		console.log(`${g} ${assertionsPassed} assertions in ${tests.length} tests all passed on ${sayNow()} ${g}`)
	}
}
...
const noop = (() => {})//no operation, a function that does nothing
...
function toss(note, watch) {//prepare your own watch object with named variables you'd like to see
...
const logRecord = "";//all the text log has logged
const logRecordLimit = 50;//until its length reaches this limit
const logDestinations = [console.log];//all the loggers we'll log to
function addLogDestination(f) {//takes another function like console.log we should log to
...
function say(...a) {//turn anything into a little text, always know you're dealing with a string
	let s = '';
...
function see(...a) {//TODO, uses typeof and stringify
...
function sayNow() { return sayTick(now()) }
