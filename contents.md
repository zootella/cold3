...
Three pieces of data-layer work are scoped and ready to pick up, and a fourth document sets the direction all of them now run toward. Two of the three wait in data.md's queue. The smaller-dog logging simplification, planned in smaller-dog.md, shrinks the Datadog apparatus to one async dog; the ledgerAdd conversions that lead it are two of three in, leaving the turnstile site, which waits on the sprint's own layering decision, and then the removal pass. The backup-plan decision chooses among the three approaches database-stack.md's bookmark presents — managed backups, the pg_dump pair, the CSV cold copy — and settles the cadence, and where the sensitive files sleep. The third is newly scoped in its own document rather than in the queue: data-cleanup.md removes address_table and service_table, once the intent recorded in their scaffolding is either built into their successors or waived on the record.

Above all three sits data-plan.md, which says where the data layer is headed — ledger_table carrying the history, every other table going back to ordinary edit-and-delete. It settles no individual question, but it is the frame each of these sprints, and the larger schema passes after them, now fit into.
...
## data-cleanup.md — removing the two superseded tables

The sprint that takes address_table and service_table out of the database and out of the code. Both were superseded by tables we since built and use, so the end of the work is small — one table dropped in the cloud, one SQL() block deleted from the registry, eight functions and a scattering of ttds removed with them. Most of the document is the reason it can't just be done: the comments, essays, and notes files around those two tables are the only written record of what they were for, and their successors can carry every one of those jobs without yet doing so. So the document captures both designs from the scaffolding, then checks each thing they wanted against what credential_table, trail_table, and ledger_table actually record today, and lists what's left — the all-types view of a user's credentials that credentialGet still stubs, the turnstile and upload and chain-provider calls that no audit row covers, robin's unmeasured user-completion times, and the OTP code riding in a ledger note in plaintext. Each becomes a deliberate build-it or waive-it rather than a silent loss. It also holds the three competing patterns for making a row go away — delete it, write a later row marked removed, or set hide nonzero — sized rather than settled, since address_table's event vocabulary is the clearest surviving statement of a pattern the codebase documents in two schemas and practices in none.
...
**By scope, narrowest to broadest:** one credential type — oauth.md, otp.md; the whole credential system — credential.md, map.md, first-night-accounts.md; the whole application data layer — data-plan.md over ledger.md, with database-stack.md, jsonb.md, migration.md, and testing.md as their reference shelf, and data.md, smaller-dog.md, and data-cleanup.md as the work waiting beneath them; and outside all of it, digital-authentication.md, a survey of what everyone else does.

**Common threads:** the planning documents all involve credentials, with the database references standing beneath them. The OTP integration that otp.md planned and credential.md framed landed in July 2026; the brownie stood up and took both flows' provisional state in August, resolving the relocate-or-eliminate fork in favor of notes — provisional state lives in the brownie, durable credentials in credential_table, and no temporary cookie remains. first-night-accounts.md builds on credential.md's early-userTag and visitor-first material, carrying the signup-side design into its own document. Underneath all the others is the data layer, where data-plan.md now sets the direction that ledger.md framed as a choice, and every credential type rests on the shape those two describe. data-cleanup.md is the first piece of work read in that new light: what it finds missing in credential_table and the ledger gets built or waived under data-plan.md's rule, not under the ledger-style assumptions the missing pieces were originally specified against.
