...
The credential system flows through one endpoint (`/api/credential`), one store (`credentialStore`), and one pattern. Email and phone one-time codes migrated in from the old `/api/otp`, which is deleted.
...
**Envelopes unified, then retired.** The provisional state of every multi-step flow rides a Challenged. row in credential_table, found by userTag: the totp secret and the wallet nonce in the row's json, the otp answer in the trail as a hash. The brownie, one localStorage entry holding a sealed letter of notes, carried totp enrollments and otp challenges from August 2026 until September 7, 2026, when brownieless.md moved them onto rows; it stays in the code, empty and running, until anonymous-users.md decides where a person with no account keeps their state, since a browser-held letter is one of that document's candidates. No temporary cookie exists, and the browserTag is cold3's one cookie. The sudo hour, when it comes, is a row or a column on the Browser. row rather than a note.

**Reducing fetch calls is the goal.** A page load should be one GET to the credential endpoint. That one response tells credentialStore everything: which credentials exist, what their display values are, and whether any multi-step flows were interrupted. Components render from the store. When a user takes an action (enroll, remove, prove), that's one POST, and the response includes a fresh attachState snapshot so the store stays in sync. The number of fetch calls should be proportional to the number of user actions, not the number of credential types.
...
**Email/phone one-time passwords** ride the same endpoint and store, and hold no cookie of their own. Live challenges are Challenged. rows in credential_table: `credentialOtpGet` projects the viewer's own live challenges beside her address list from the one query it already runs, `attachState` builds `task.otps` from them as the non-secret display parts, and the answer is a hash in the trail. The store reads that snapshot and holds nothing durable itself. Recovery after a refresh needs no round trip at all: the server render paints the enter boxes on first paint, and every response carries the full picture. The store's mounted follow-up still asks `brownieHeld()` and fires only for a browser holding an old letter, fetching nothing new; it leaves with the brownie's removal.
...
## Provisional state that survives reload

These are multi-step flows where the user might refresh the page mid-enrollment, so the state between steps has to survive a reload — both as Challenged. rows in credential_table, found by userTag, so any browser signed in as the user can finish.

**TOTP** — 2-step enrollment. `TotpEnroll1.` generates a secret and writes it into the json of a `Totp.` Challenged. row, hiding any earlier start. `TotpEnroll2.` reads the start, validates the user's 6-digit code, and saves the secret as the Proven. row; `TotpClear.` hides the start. One enrollment per user at a time, twenty minutes from the row's row_tick, checked at every read. Recovery: `credentialTotpGet` returns the enrollment beside the proven secret from one query, `attachState` carries it in every snapshot, and the page renders the enrollment whenever the snapshot carries it, on first paint included.

**OTP** — 2-step challenge (send code, enter code). `OtpSendTurnstile.` generates a code, writes its hash to the trail, and writes an `'Email.'` or `'Phone.'` Challenged. row carrying the provider and the challenge's tag, one row per live challenge, hiding the earlier live challenge to the same address. `OtpEnter.` finds the row by the user and the tag, checks its twenty minutes, and hashes the guess into the trail's answer message. **Multiple simultaneous challenges** — several rows stand at once, one per address per user; a user might authenticate both their email and phone number at the same time. Recovery rides the same snapshot: `task.otps` carries the non-secret display parts on every response.

## No reload to survive

These flows hold nothing across a reload, because a reload restarts them.
...
**Wallet** — no envelope. Prove step 1 writes a Challenged. row with the nonce and the connector in its json and hands the page the nonce, which the page embeds in the SIWE message the wallet signs; step 2 finds the challenge row by the nonce in the signed message, verifies the signature, hides the challenge, and writes the proof with the nonce that proved it. The signing happens in-page via a wallet popup (MetaMask, etc.), so there's no navigation away, and a refresh restarts the flow because it kills the pending signature request.

## Unification direction

Resolved, twice. In August 2026 the brownie held it all, totp's singleton as one note per user and otp's multiplicity as one note per challenge. In September 2026 brownieless.md superseded that with rows: every flow's provisional state is a Challenged. row found by userTag, with no browser binding, and the brownie stands empty pending its removal. That is what runs today.
...
- Event 4 (proven): credential is active — the user has a password, a TOTP enrollment, a browser sign-in
...
**Reopened, September 2026.** The fork was decided days before jsonb entered the menu, and the objection that carried it — a dedicated table or sparse columns per type — does not apply to a json cell on a row that already exists. brownieless.md took the fourth step in September 2026: the provisional state of all three flows became json on credential_table's Challenged. rows, found by userTag, and the letter carries nothing. The brownie itself stays in the code, empty, until anonymous-users.md decides whether a person with no account keeps their state in the database or in the browser; the removal is written and waits on that.

## Browser binding: a brownie rule, not a row rule

A flow that rides the brownie must be started and completed at the same browserHash. The letter is a bearer token in the client, so it is bound to the browser at the door — `openBrownie` wipes the notes of a letter whose sealed browserHash disagrees with the one the request's cookie proves — and no flow re-checks it. A flow that rides a Challenged. row has no such need: nothing presents a row, the query is by user_tag, and the signed-in session a request does present is already tied to the user by the Browser. row. All three flows moved to rows in September 2026 and bind to no browser: any browser signed in as the user shows the qr code and can finish, a wallet's challenge is found by the nonce in the signed message with the signature as the proof, and an otp challenge is found from the user by its tag with the code in the inbox as the proof — so "requested on laptop, reading email on phone" now finishes on the phone. With no flow left in the brownie, the letter's browser binding guards nothing, and leaves with the removal pass.
...
**Two sprints, one seam.** The brownieless sprint of September 2026 codes every credential flow for a person who already has a userTag: the endpoint resolves the signed-in user from the Browser. row and hands the flow a userTag, and the flow functions in level3 take a userTag and never a browserHash. That is the add-a-credential direction, the one that runs today. Sign-up is the other direction and a later sprint: a stranger arrives at a browser with no userTag, and the work is the step above the flow functions, minting the tag and writing the Browser. row at the first moment there is something to remember, then calling the same flow functions with it. Keeping the flow functions ignorant of browsers is what makes that seam clean: sign-up changes who calls them, not what they do. otp was built first and with sign-up in mind, which is why a browserHash once reached its functions; brownieless removes it, and the sign-up sprint puts the browser back where it belongs, above the seam.
...
**TOTP** — note={secret} (base32; a key, not a hash, so it rides the note). Two-step enrollment with the provisional secret riding the json of a Challenged. row (September 2026; the brownie note and the envelope cookie before it are retired). UI in TotpPanel with sub-components (TotpInput, TotpText1, TotpText2). Mobile detection via `browserIsBesideAppStore()`. The flow lives in level3 (`credentialTotpEnroll1/Enroll2/Clear`, with `credentialTotpGet` returning the enrollment in flight) so grid tests can walk it against rows; the endpoint branches are thin. `credentialTotpGet/Set/Remove` for the stored enrollment.

**Wallet/Ethereum** — the address rides the f triad per `validateWallet`: f0=lowercased to match as unique, f1=f2=the EIP-55 checksummed face; zero, one, or two proven addresses per user, held as peers, and no two users can hold the same one. Two-step prove flow with no envelope: step 1 hands the page a nonce that the signed message carries back, and step 2 finds the challenged row by it (no cookie — signing happens in-page via wallet popup). UI in WalletPanel, wagmi lifecycle in wagmiStore (Pinia), wagmi loaded on demand via `wagmiDynamicImport()` and viem via `viemDynamicImport()` (viem is full-stack — the worker validates SIWE signatures with it too). Two connectors: injected and WalletConnect, and the challenged row records which one, beside the nonce; the proven row records the nonce of the challenge that proved it. First credential type to write events 2/3 (mentioned/challenged in WalletProve1). The prove flow lives in level3 (`credentialWalletProve1/Prove2`) with the cap and one-holder rule enforced by `credentialWalletRefusal`/`credentialWalletHolder`; `credentialWalletGet/Set/Remove` round it out.
...
**Email/Phone (OTP)** — any number of addresses per user, all peers, no main or default. f0/f1/f2 = normalized/formal/display forms from `validateEmailOrPhone`; type `'Email.'` or `'Phone.'`. Each address's lifecycle is event rows: Mentioned., Challenged. (json={provider, tag}, `Amazon.`/`Twilio.`, so time-to-prove per provider is queryable), Proven. (json={tag}, naming the challenge that proved it) — current status is the highest visible event, and remove hides the whole lifecycle. A proven address is held: no other user can be challenged at it or claim it (outcome `Held.`, checked at send and again at enter to close the race where two users held live codes). Otp flows require a signed-in user from send through enter, full stop — each Challenged. row belongs to the userTag that started it, and enter finds only hers (outcome `Expired.` for anyone else's tag, `SignedOut.` with no session at all) — until the early-userTag design opens these flows to signup. Two-step challenge with each live code a Challenged. row of the user's, several at once, the answer a hash in the trail; all codes are entered in the TopBar `OtpEnterList` box, one enter system for demo and credential flows alike. UI in EmailPanel and PhonePanel. One level3 family does all of it, taking type as a parameter: `credentialOtpSend/Enter/Get/Remove/Holder/Mentioned/Challenged/Proven` (the endpoint resolves the signed-in userTag and passes it down; no browser binding, so a code requested on a laptop can be entered on a phone signed in as the same user).

## Future wallet types, mapped

The road ahead for cryptocurrency credentials, positioned by the August 2026 f-triad repair. Ecosystem neighbors like Doge. or Solana. arrive as new type_text values with their own validate and prove flows — different address formats, different signature schemes, genuinely new ways to sign in, which is what type_text exists for; f0 is whatever exact string that type's validate mints for matching. EVM network variance like Base is proof metadata, not identity: an address is the same address on every EVM chain, and an ordinary wallet's signature proves key control regardless of chain. The one place chain matters is the smart-contract-wallet corner, where EIP-1271 verification asks a contract deployed per chain — so if we ever verify proofs against multiple chains, challenge and proven rows grow note {chain}, absent meaning mainnet. Neither future needs a migration.
...
Then Alfred completes the proof — and the story rewrites itself. The address was Alfred's all along; Alice had typed her own address wrong. Alfred now holds al@gmail.com as a proven credential. The table still remembers that Alice mentioned it — nothing is erased — but the address belongs to Alfred.
...
**Known third surface, deferred: the page has no clock on provisional state.** A totp start and an otp challenge each live twenty minutes, and the server enforces that at every read. The page knows nothing of it. It draws the qr code or the enter box from the last snapshot and keeps drawing it until another snapshot says otherwise, so a user who was interrupted and comes back an hour later learns the flow died only by typing a code, hearing Expired., and watching the ui collapse with no words — the same experience as cancel, which the September 2026 smoke of the distracted-user path confirmed. A more sophisticated page would carry each provisional item's deadline in the snapshot, row_tick plus the twenty-minute constant, and at that moment either fall back silently to not-started, or show a line saying this code has expired and offering to start again. Either is one rule for every provisional flow, totp and otp alike, and wallet if its flow ever resumes across a refresh, so it wants doing once, en masse, rather than per panel, and nothing blocks it now that every flow's provisional state is a row with a row_tick to read the deadline from. The server stays the clock either way; the page's timer would be a courtesy to the person, never the enforcement.
