...
A flow that rides the brownie must be started and completed at the same browserHash. The letter is a bearer token in the client, so it is bound to the browser at the door — `openBrownie` wipes the notes of a letter whose sealed browserHash disagrees with the one the request's cookie proves — and no flow re-checks it. A flow that rides a Challenged. row has no such need: nothing presents a row, the query is by user_tag, and the signed-in session a request does present is already tied to the user by the Browser. row. totp and wallet moved to rows in September 2026 and bind to no browser: any browser signed in as the user shows the qr code and can finish, and a wallet's challenge is found by the nonce in the signed message, with the signature as the proof. otp decides for itself when it moves, and it is where the "requested on laptop, reading email on phone" question lives — the code typed back at the browser that asked is simpler, and brownieless.md weighs whether it is also worth keeping.
