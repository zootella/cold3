...
First in the queue as of September 4, 2026, ahead of the smaller dog, and needing no migration. Retire the brownie, planned in brownieless.md: the totp secret and the otp answer move from the sealed letter in localStorage into the database, beside the credential_table rows that already record the start of each flow — otp's event-3 row grows its json, totp gains an event-3 row it never had, and the otp answer rides the trail as a hash — and then the letter, the door's open and seal, the page's carriage and follow-up, the note vocabulary, and the wallet flow's envelope all leave. The document holds the no-session principle behind it, the table-cookie-letter trajectory and why json changes the answer, what the database already records, the shape after for each flow, and the decisions, all made, with the two-query snapshot deferred to its own exploration after the credential_table pass. The document opens with how the three flows work today in every place their data lands, with example rows shaped from the hosted database, and repeats the diagrams with the sprint's changes marked. Four steps: the otp move, the totp move, the wallet move, the removal.
