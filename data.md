...
1. The expansion migration, pushed September 6, 2026: event_text is on the cloud table, all 262 rows blank in it, and event has its default. Add event_text as TEXT NOT NULL with a default of blank, so rows written by the old code stay whole. Give the old event column a default too, 0, so that when the code later stops sending it, inserts still work. No index names event, so no index changes. The registry in level3.js shows both columns in the same commit.
