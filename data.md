...
First in the queue as of September 5, 2026, ahead of brownieless. The word for "the user proved control of this address" was validated in some places and proven in others, and it becomes proven everywhere, in two passes. The first pass: comments and documents are done; the code and the data are this step. The oauth handler writes Proven. instead of Validated. into ledger_table, the function credentialOtpValidated becomes credentialOtpProven, and one migration file updates the five ledger rows that say Validated. to Proven. That migration changes no column, so it is one push, not an expansion and a contraction. The code deploys first and the push follows, so a sign-in between the two is caught by the update. The second pass: credential_table's event column, which holds the numbers 2, 3, and 4, becomes event_text holding Mentioned., Challenged., and Proven., by expansion and contraction on migration.md's playbook.
