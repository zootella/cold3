...
**The re-proof, one of those flows.** A proven address is a fact that goes stale: people change numbers and abandon mailboxes, and a recovery channel nobody has checked in a year is not a recovery channel. The flow the mature apps run is a gentle, periodic one, always on the happy path, something like a prompt after twelve months: "Is alice@example.com still your email address?" with one button that says yes and one that says no and opens the change. Yes sends a code and she enters it; no removes the address and starts a fresh proof of the new one. The same mechanism serves the sudo check, proving a held address again before something that matters, and the strengthening ladder, where a credential re-proven recently counts for more than one nobody has touched in years. The table side of this exists as of the hideless sprint: a proof and a live challenge for one address stand together, finishing the challenge edits the standing proof to the new face and the tag that proved it again, and the ledger holds every proof with its moment, which is where "twelve months since" is read from. What is not built is everything above that: the prompt, its cadence, the notification that carries it, and the rule for what a stale proof is allowed to do.
