...
1. **The ledgerathon lands.** Every mutation writes its row before any credential row is deleted, so the first rows to go are rows the ledger already describes better. Done September 13, 2026. The week of soak the plan asked for was set aside the next day for a code review of the pair mechanism, and the orphan query, an Asked. row with no Answered., is the standing check.
...
6. **The contraction push.** Drop the hide column from every cloud table, which takes the seventeen old indexes with it, and drop the scaffolding default. The SQL() blocks show the final shape in the same commit, and a drift check closes it, columns matched by name and exact inside indexes.
