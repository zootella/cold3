...
**Where we are, September 3, 2026.** The first three tasks are done: ledger_table has its vocabulary and json, credential_table's note_json is json with credential15 carrying the oauth claim, every contraction is pushed, and the drift check is clean across every cloud table. Nothing is in flight. Next is the fourth task, moving hit_table's rows into ledger_table as Hit. records, starting with the ledger7 expansion migration and the recordHit rewrite.
