...
/*
The brownie is a secure place for the user at a browser to keep state from the server--a sealed letter parked in localStorage that survives refreshes and spa lifetimes, opaque to the page, which can neither read nor forge it. It is not authentication and not a JWT: the browserTag cookie identifies the browser, the Browser credential row links it to a signed-in user, and the brownie holds only what's ephemeral--the provisional records of in-flight flows, like a totp secret between the qr scan and the first code, the codes of live otp challenges, or someday sudo permission, each note labeled to die in about twenty minutes. It's the sealed-session lineage (Rails CookieStore, h3's own useSession) rather than a session store on KV or Redis, because ephemeral state deserves neither a second database nor Supabase's durability--with three reasoned deviations: localStorage instead of a cookie (the ciphertext rode every request's headers for nothing, and 4KB caps growth), explicit body carriage the server reads on purpose, and per-note owners, which neither lineage has.

The letter carries browserHash and notes, and each note names its own type, expiration, and owner. That one shape does most of the work: housemates sharing a browser profile share the letter, notes mixed, and each flow touches only notes whose userTag matches the user it resolved--so Alice finishing her enrollment reseals a letter still carrying the note Bob needs, a note can be owned by nobody yet for sign-up flows before a tag exists, and nothing in devtools reveals who is mid-flow, since owners live only inside the ciphertext.

On the wire, fetchWorker sends the held ciphertext up with every POST, and the response commands a complete replacement (BrownieSet.), a delete (BrownieDelete.), or says nothing--replace-always, never an edit. Silence is load-bearing in both directions: a response to a request that departed brownie-less, or that carried the letter and left it unchanged, must say nothing, or its late-landing stale reply would clobber what the page stored after it departed. The one race that remains--two tabs mutating at once, last writer wins--is accepted deliberately: it costs at worst a restarted twenty-minute flow, and every flow re-validates against the database at the moment it matters.

The door never tosses over a brownie. A poisoned entry would otherwise blow up every page load until someone cleared localStorage by hand, so every failure at open--junk that won't decrypt, a sealed browserHash that disagrees with the one the request's cookie proves, an old letter shape from before a deploy, a malformed or expired note--degrades to an empty letter, which the response turns into BrownieDelete., cleaning the browser up in one request.

To move a new flow in: the door hands request code the open letter as door.brownie--set your note with a type, an expiration, and an owner through the brownie vocabulary below, and the door seals whatever the letter holds on the way out. That's the whole contract.
*/

function brownieRead() {//the sealed ciphertext this browser is holding, or blank for nothing--text or blank; tabs share the entry and read fresh at each use; private to level2, with fetchWorker and brownieHeld its only callers
	try {
...
function checkQueryCell(title, cell){//in a column with the given title, check the value in a cell
	...
	else if (type == 'json') checkQueryJson(cell)
...
test(() => {
	checkQueryRow({
		...
		'preference': -1,
		'settings_json': {sound: true, volume: 7},
	})
})
test(() => {
	...
	checkQueryCell('geography_json', {country: 'US'})//because column title ends "_json" cell value must be a plain object
	checkQueryCell('json', {})//a column can be titled just json, the way a column can be titled just hash; blank is the empty object
})
test(() => {
	ok(_type('name_text') == 'text')
	ok(_type('other') == 'other')
	ok(_type('longer_name_hash') == 'hash')
	ok(_type('browser_json') == 'json')
	ok(_type('json') == 'json')//no underscore returns the whole title, which is how bare json and bare hash columns resolve
})
test(() => {
