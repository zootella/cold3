...
- **Integers past 2^53** — refused, though not for the reason it first looks like: every finite double survives print and parse exactly, since JavaScript prints the shortest text that round-trips. The loss happens earlier, when the source is read — the literal `9007199254740993` is already `...992` before JSON is involved — so storing one records a number nobody wrote. Big identifiers ride as strings. (Our millisecond ticks, around 1.7e12, are far inside the safe range.)
...
How that dedup actually works, traced: `recordHit` hashes the row app-side before `row_tick` is added, stores the result in the neighboring `hash` column, and the `UNIQUE (hash)` constraint plus `queryAddRowIfHashUnique` swallow the duplicate. The hash is minted once, at write, from values the worker holds.

The guarantee we want is that a value can move between object, text, jsonb, and hash in any direction, any number of times, without ever looking different when it is the same or looking the same when it is different. Three helpers in core carry it, and each answers one half-truth that would otherwise bite.

**A value must survive our own printing unchanged — `isPlain`.** A plain-data walk refuses everything stringification would quietly convert, naming each lie precisely: BigInt prints as numerals, NaN and Infinity print as null, an integer past 2^53 is already a different number before printing begins, a Date or class instance prints as something else entirely, a circular reference prints as a marker. Then the round trip stands behind the walk as the closing certainty — print, parse, print again, and require identical text. The walk alone would miss values that stabilize *after* one silent conversion (a Date round-trips stably once it has become a string); the fixed point alone would bless that conversion. Together they are `isPlain` in core, with `checkPlain` beside it and the unit tests as the contract's demonstration. This sits exactly beside is-it-an-integer and is-it-text, and level2's `checkQueryJson` is the query layer's member of that family, asking `isPlain` and tossing like its siblings.

**A hash must name the data, not the order it was built in — `hashObject`.** `{a:1,b:2}` and `{b:2,a:1}` are the same data and print differently, so a hash over plain text is a hash of assembly order. `hashObject` closes that: it calls `checkPlain`, sorts the keys at every level through the private `sortObjectToText`, and hashes the sorted text. Equal data always gives an equal hash. The sorting happens in a copy, so the caller's object is untouched.

**Two sorts are at work in this stack, and they never have to agree.** Postgres orders jsonb keys its own way, by length and then bytes; we order ours plainly in JavaScript's string order. That difference is harmless because neither side ever compares its text against the other's — each sorts only to compare within itself, and a value crossing the boundary crosses as *data*, since a jsonb column hands back a parsed object rather than a string. It re-enters our layer as data and sorts our way again. Reproducing Postgres's exact algorithm would be a false promise anyway: it also normalizes numbers and collapses duplicate keys, so text-level agreement isn't something we could guarantee. Data-level agreement is, and that is what we rely on. The one thing to avoid is comparing our printed text against Postgres's printed text.

Which retires the old rule. It used to be that a hash could never be recomputed from a read-back jsonb cell — jsonb genuinely does not preserve text, so the recomputed hash would silently differ. That was a rule you had to remember. `hashObject` makes it a property that holds anyway, proven by a grid test that sends an object into a real jsonb column, reads it back, shows the plain texts differing, and shows the hashes agreeing. We still hash at write and compare stored hashes, because there is no reason to recompute what you already have — but an accidental recompute is no longer a silent wrong answer. (Postgres's other type, plain `json`, preserves exact text but forfeits the binary operators and indexing; we chose jsonb and accept its normalization.)
...
**Done, August 18.** Pushed clean; the CLI's `supabase_migrations.schema_migrations` table appeared with it, recording each migration's version, name, and the statements it ran.
...
**Done, August 18, and the verification came out better than planned.** The catalog showed the four columns in their expected shape. But the visit that was supposed to produce a fresh row produced none — the hourly dedup swallowed it, same browser and same values inside the same hour. What proved the write path instead was delay_table: report.js awaits recordHit *before* recordDelay, so a delay row can only exist if recordHit returned without throwing, and there was one. That also proved the defaults were doing their job, because the deployed code's INSERT never mentions the json columns, and a missing default would have raised a not-null violation, which `queryAddRowIfHashUnique` does not swallow — it only swallows the duplicate-key code. Three ordinary facts, and together they said the schema and the running site were coexisting exactly as expand intends.
...
**Done and deployed, August 18.** recordHit takes `geography` and `browser` as objects, report.js passes them straight through, and the first hit_table grid test landed beside the change. Before deploying, a local smoke test closed the last unproven link: every earlier write proof had gone through PGlite's adapter, which binds an object as printed text, while the real path has supabase-js carry the object in a PostgREST request body — different mechanisms for the same job. Local dev talks to the real hosted database, so one page load wrote a row with real `browser_json` and read it back parsed through the app's own helper. The deploy followed, and a visit to the live site produced `{"city": "New York City", "postal": "10004", "region": "NY", "country": "US"}` — four Cloudflare headers that used to be flattened to a string at the report.js seam, now riding as an object. Postgres stored those keys in its own order, city before postal before region before country, which is length-then-bytes and a live demonstration of why `hashObject` sorts before hashing.
...
**Written and committed, not yet pushed.** The migration file carries the backfill first, then the four ALTERs, and level3's registry already shows the final shape. One thing the writing turned up: the backfill must be guarded. There were 2,808 rows holding text and 3 written since the deploy holding a blank, and `''::jsonb` is a syntax error, so an unguarded UPDATE would fail the whole migration on the newest rows. Each statement carries `WHERE geography_text != ''`. A read-only check confirmed all 2,808 cast cleanly to jsonb objects before the file was written.
...
**The lockstep rule threads every phase**: each migration file and its SQL() registry edit land in the same commit — the expand commit shows both column sets with their temporary defaults, the contract commit shows the final shape — so the registry mirrors production at every point in git history, and grid tests exercise each phase's code against that phase's true schema. Mirroring stops short of column order (decided August 18, after briefly deciding the opposite). Postgres appends new columns at the physical end and has no way to move one, so a registry that mirrored physical order would slowly become a record of the order things were historically added rather than a description of what the table means — and the SQL() blocks are the schema's documentation, read far more often than the dump. So the registry orders columns for legibility, freely and at any time, and the drift check matches columns by name instead of by position. Nothing is lost, because column order carries no meaning: everything here reaches a column by name, and the alternative would mean rebuilding a production table for aesthetics every time a block reads better rearranged. One sharp exception — column order *inside* an index or constraint is meaningful, since a b-tree on (a, b) is not a b-tree on (b, a), so the check stays strict there.
...
1. **jsonb becomes an approved type in icarus.** The check function, the dispatch branch, the grid spike proving PGlite and the adapter handle objects, the essay and comment updates. This lands and deploys on its own and changes no live behavior — the spike's json column lives on example_table, which exists only in PGlite, so the capability arrives fully tested with zero cloud contact.
   ...
   **Done, August 14.** Landed as scoped: `isQueryJson` and its walker in level2 with the demonstration and refusal unit tests beside them (step 3 moved all three down to core as `isPlain`, where recordHit needs the same answer), the `'json'` dispatch branch, example_table's `some_json JSONB` column, the five existing grid inserts carrying the explicit `{}` blank, the adapter binding objects as printed text, two grid tests proving parsed round-trip, canonicalized keys, and refusals at the write path, and tables.txt telling the type's story with its siblings. The known unknown resolved the good way: PGlite parses jsonb back to objects natively, so the read side needed nothing.
2. **The live database migration** — the expand phase and its verification, per the choreography above.

   **Done, August 18**, and preceded by a migration that wasn't originally part of this sprint: row-level security, enabled on all six cloud tables with zero policies, plus a REVOKE of the anon and authenticated grants. That went first deliberately, as a genuine gap-closer that doubled as a zero-risk rehearsal of migration-new-then-push before hit_table's turn. The reasoning is in database-stack.md; `supabase db advisors --linked` now reports nothing about RLS.
3. **The code refactor** — recordHit and report.js write objects, deploy, watch the dashboard; then the contract migration and the closing drift check finish the sprint.
   ...
   **Done and deployed, August 18. The contract migration is written and committed but not yet pushed, and the closing drift check waits behind it.** The code step grew beyond its scope in a way worth recording, because the growth was the sprint teaching us something. Writing recordHit's check raised the question of where a json value's check belongs, which pulled the walker and its round trip down from level2 into core as `isPlain` and `checkPlain`, sitting beside `makePlain` — one forces a value into plain form, the others ask and refuse instead. Then the hash question: `{a:1,b:2}` and `{b:2,a:1}` are the same data and print differently, so a hash over plain text names the order a value was assembled in rather than what it holds. That became `hashObject`, which checks the value is plain, sorts the keys through a private printer, and hashes that — which in turn retired the old never-rehash rule, since a hash taken from a read-back jsonb cell now matches. And noticing that level2 had its own hand-rolled integer check led to `minInt` collapsing from twenty-five lines to one, verified equivalent across 1.8 million pairs and portable by specification rather than by testing on one engine. None of that was planned. All of it came from the first real use of the new cell type asking questions the spike hadn't.
