...
delay_table's d1 through d5 look like the k1–k8 smell but pass the test above and stay numeric columns: the table exists for SQL aggregation — percentiles over durations, postgres doing the math — which makes the slots margins, not payload. Their per-task meanings never mix, because every slot is always a duration in milliseconds and aggregations group by task_text.
