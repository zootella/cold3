...
Read it that way: not as an open question waiting for a verdict, but as the reasoning the verdict was reached against, and the place to come back to if the first real migration turns up something the argument missed. What follows lays out both patterns and their costs so a single rule can be applied everywhere — likely as a focused sprint, not an incremental change. That sprint is now planned: hideless.md converts the tables, and the ledgerathon, done September 13, 2026, wrote the audit rows the conversion depends on.
