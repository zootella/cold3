...
**Does the prove flow need the page to hold server state?**
For instance, Wallet: prove step 1 hands the page a nonce, which the page embeds in the SIWE message the wallet signs, and step 2 finds the challenge row by that nonce, so the page holds nothing but the nonce, in a local variable, for the seconds between the steps. Until September 2026 the nonce rode a sealed envelope the page sent back with the signature. Either way a browser refresh restarts the flow, because it kills the pending signature request and the panel disconnects an unproven wallet on mount; the user reconnects with a click, or a rescan for WalletConnect.

**Does the prove flow need that state to survive a browser reload? (by a Challenged. row in credential_table since September 2026)**
...
No, for every type, since September 2026: each start is a row that belongs to the user, so any browser signed in as her shows the qr code and can finish, any browser signed in as her can submit a wallet's signature over the nonce she was issued, and any browser signed in as her shows the enter box and takes the code. For instance, OTP: request a code from your laptop, read the email on your phone, and type the code on the phone, where the enter box is waiting too, because the challenge belongs to her and not to a browser. Single-browser was the brownie's rule, because a letter in localStorage is a bearer token that had to be bound to something, and a row is not.

**Where is this flow's expiration enforced?**
Provisional state must die on its own in tens of minutes — the concern is who checks the clock. For instance, TOTP: the start's row_tick plus twenty minutes is the deadline, checked in the one function in level3 that reads a start, so a stale start never reaches the page, which learns on its next snapshot. OTP: the same, the challenged row's row_tick plus twenty minutes, checked in enter and in the snapshot's projection. The page has no clock on any of it: it draws the qr code or the enter box from the last snapshot and learns a flow died only when the next snapshot omits it, which credential.md's page-response section holds as a deferred page-side improvement.
...
- **Date of birth** — floated in user.txt as pre-proof matching information for the lazy-signup flow, then dismissed in the same note in favor of requiring the code on the second device. Revived July 2026: the one-thumb signup design pairs DOB with the phone number as its knowledge factor.
...
**Proven addresses are unlimited, and so are simultaneous challenges, by choice.** Every live otp challenge is its own Challenged. row, one visible per address, and nothing counts them — the real bounds are a turnstile solve per send and the per-address trail limits, which cap how fast challenges can accumulate. The old accidental ceiling (about a dozen challenges filling a 4KiB cookie) left with the cookie.
