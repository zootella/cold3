...
log, look, Data, Now, Tag, tickToText,
...
process.loadEnvFile()//put .env properties on process.env
...
async function listFiles() {
	...
	let paths = await glob('**/*', {
		...
		ignore: [//exclude these folders and files from getting listed and hashed in wrapper.txt
			...
			'**/.temp',//machine-local tooling state by name; the supabase cli caches versions of its hosted services in supabase/.temp, refreshed on connect
			'**/.branches',//supabase cli local branch state
...
async function writeReadme(o) {
	...
	let body = lines.slice(fence2 + 1).join('\n')
	let s = '\n' + '```' + disk + '```' + '\n' + body
...
async function affixSeal(properties, manifest) {
	...
	for (let f of properties) {
		...
		if (
			...
			!f.path.endsWith('.md') &&//markdown documents, human-readable and responsible but generated by Claude 🖍
			!f.path.endsWith('grid.js') &&//the grid integration suite, hashed as identity but not counted as reader weight; it grows and grows, and gets a different level of scrutiny than code and the unit tests inline beside it
			!f.path.includes('site/size/') &&//rollup visualizer reports
			!f.path.startsWith('supabase/')) {//cli scaffolding and applied migration history; the readable schema truth is the SQL() registry in level2 and level3
