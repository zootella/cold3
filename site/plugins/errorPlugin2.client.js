...
export default defineNuxtPlugin((nuxtApp) => {
	nuxtApp.vueApp.config.errorHandler = (error, instance, info) => {
		...
		nuxtApp.error({statusCode: 400, source: 'Handler.', error, instance, info, message: 'A runtime error occurred.' })
	}
	nuxtApp.hook('vue:error', (error, instance, info) => {
		//you've observed mistakes in watch and click handlers lead here
		log('error handler 2', look({error, instance, info}))
		nuxtApp.error({statusCode: 400, source: 'Hook.', error, instance, info, message: 'A runtime error occurred.' })
	})
