...
async function parentClickFunction() {
...
watch([refInFlight], () => {
	//we might need to watch for changes to in flight in the parent, too
	if (refInFlight.value) {
		//like, to animate a throbber
	} else {
		//or stop that animation--this is just a contrived idea as to a reason why
	}
})
</script>
<template>
	<div class="border border-gray-300 p-2">
		...
		<p>
			...
			<PostButtonComponent
				...
				@click-event="parentClickFunction"
			/>
		</p>
		<p>the button is in flight right now: {{refInFlight}}</p>
	</div>
</template>

here's my current draft. please check for mistakes. parent form component above, and child button component below.
...
watch([() => props.inFlight], () => {

	refButtonState.value = props.inFlight ? 'orange'          : 'green'
	refButtonLabel.value = props.inFlight ? props.labelFlying : props.labelIdle
