...
async function runOauth(event) {//the flow itself; the membrane above runs this for every url under /api/oauth/* — Auth.js routes on the path internally (/api/oauth/signin/discord, /api/oauth/callback/discord, ...)
	...
	let authOptions = {
		...
		callbacks: {
			...
			async signIn({account, profile, user}) {//Auth calls our signIn() method once when the user and Auth have finished successfully with the third-party provider
				...
				await ledgerAdd({action: 'Oauth.', event: result.ok ? 'Validated.' : 'Refused.', provider: providerInfo.tag, browserHash, userTag: signedIn.userTag, ip: toTextOrBlank(headerGetOne(event.req.headers, 'cf-connecting-ip')), note: makePlain({account, profile, user, outcome: result.outcome})})//the person proved control at the provider either way; the event says whether we took the proof, and makePlain deliberately flattens whatever provider-specific shape came back into plain data the note can hold
	...
	if (errorType == 'Configuration') {//our bug to fix. Configuration is also @auth/core's fallback bucket for any non-client-safe error (a bad secret, a malformed provider, our own signIn callback throwing), so it catches the unexpected too. (add more types here if auth.js surfaces other our-fault ones)
		...
		await ledgerAdd({action: 'Oauth.', event: 'Cancelled.', provider: toTextOrBlank(oauthProviders().find(p => p.name == authProviderName)?.tag), browserHash: await hashText(checkTag(event.context.browserTag)), ip: toTextOrBlank(headerGetOne(event.req.headers, 'cf-connecting-ip')), note: makePlain({errorType, path: event.path, error: authError})})//the provider is blank when the flow broke before the path named one
