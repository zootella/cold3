	...
	let authOptions = {
		...
		callbacks: {
			...
			async signIn({account, profile, user}) {//Auth calls our signIn() method once when the user and Auth have finished successfully with the third-party provider
				...
				let result = await credentialOauthSet({userTag: signedIn.userTag, ...parsed})//writes the credential row and the Oauth. ledger row beside it, Proven. or Refused. with the outcome
				...
				log('Auth.js signIn() handler', look({account, profile, user, outcome: result.outcome}))//operational visibility in datadog, separate from the ledger row credentialOauthSet wrote
