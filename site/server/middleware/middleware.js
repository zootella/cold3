...
Confirm in results:
- **`x-card-source`** — `FRESH` on first fetch (~650–1100ms, full satori render), `RECYCLED` on second (~40ms, edge cache)
- **`cf-cache-status`** — Cloudflare's CDN confirmation. `HIT` on RECYCLED, absent on FRESH.
- **`age`** — seconds since edge cached (present on RECYCLED)
- **`set-cookie`** — must be absent. If present, the CDN won't cache and every request re-renders.
*/
...
async function middlewareImage(workerEvent) {
	...
	if (hit) {//yes ♻️ return it from the CDN quickly
	...
	} else {//no 🎨 call ourselves to render the card, then store it in the CDN and return it
