...
- The hulls in all three doors, collapsed to a single catch: console.error is no longer the last-resort punt, it is the channel — the provider dashboards are where deployed output lands and where broken-state investigation always really happened. It's synchronous, it needs no secret, and it works while everything else is broken, so e1, e2, and e3 become one catch, the [OUTER] label goes, and a door that fails answers 500 after one console.error. The lite door keeps its redirect to error3 for the browser's sake, with error3 logging to console the same way.
- The Error3. envelope round trip. It was born in April 2026 to carry an error from the oauth worker on its own SvelteKit domain, which had neither Datadog nor Supabase, back to the apex worker that had both, sealed so the hand-off across domains couldn't be forged. Now that the oauth handler runs inside the apex worker under doorWorkerLite, its one remaining job is reaching the standard logging path from a request with no page or fetchWorker in the loop: the lite shut seals the error, redirects the browser to error3.vue, which posts to error3.js, whose toss logs it and blows up error.vue. error.vue never shows the error, so nothing but the log ever reads the envelope. Once the lite shut logs with console.error itself, the envelope and error3.js retire; the redirect stays, because a browser mid-navigation still needs to land on a page that blows up to error.vue, and error3.vue becomes that bare landing, tossing on arrival with nothing to post.
