...
Third in data.md's order as of September 7, 2026, after the ledgerathon and hideless. Datadog can only leave once every change in the system has its record in ledger_table, and those records are only proven complete once the tables have stopped keeping history of their own, which hideless does. The three logAudit conversions at the bottom of this document were to lead this sprint. Two are in, and the third, turnstile, is decided a special case: a failed bot check is not a change to our data but a record of our experience with a third party, exactly what logAudit was for, so it gets no ledger row and forces no layering decision, and this sprint picks its channel, console.error to the Cloudflare dashboard or the one async dog that survives, to Datadog. What this document plans is the removal pass that follows.
...
- **The layering seam — decided September 7, 2026: dissolved.** Turnstile's failure is not a change to our data and gets no ledger row, so no audit crosses from level2 to level3 and the question goes away; its channel is console.error to the Cloudflare dashboard or the surviving dog to Datadog, chosen in this sprint. As it stood: ledgerAdd is level3; the turnstile check and the doors are level2, which cannot import upward. The clean resolution: alerts and door errors go to console (level2 needs no ledger), and business audits move to code that already lives at level3 or in site endpoints — the turnstile audit moving up to its callers, or the turnstile check moving up to level3, with browserHash passed down either way. Decide which.
- **Latency at audit sites — decided September 9, 2026: we accept it plainly, and build no successor.** An awaited ledger write costs ~100ms on the few paths that audit, and server code awaits every call to the database and to a third party rather than leaving one running while the response goes back. database-stack.md carries the rule and why neither ctx.waitUntil nor a parking lot of our own is the answer. The one thing that still rides keepPromise is dog, and it leaves with the parking lot; if a surviving dog stays fire-and-forget, that is a deliberate exception for a debug line we can afford to lose.
...
1. **oauth done and oauth sad path** — in, both channels writing. browserHash and userTag were already in hand and site code imports freely, so ledgerAdd sits beside logAudit at each site, landing Oauth. rows tagged Proven., Refused., or Cancelled. while the old audit still sends; the old calls leave with the removal pass. The "accumulate real examples of provider responses" motivation is a database use case that was living in a log.
2. **The message task** — in at the worker, where a Challenged. row tagged with the address type and the provider holds the whole record; the lambda still sends its own logAudit of the task, which leaves with the removal pass. The lambda returns the whole task object (provider, parameters, request, response, error, duration) to the worker through the door, so the worker calls ledgerAdd with full context and the lambda never needs database access. The http response from lambda to worker is reliable enough: losing it means the worker sees a door error and records the failure — the event is never silently lost, only, in the rarest window, the provider's detail.
3. **turnstile failure** — the one still on logAudit alone, held by the layering decision above: the check lives in level2, which cannot import ledgerAdd from level3. It converts with browserHash threaded down once that decision lands. Decided September 7, 2026: it does not convert; a failed bot check is our experience with a third party rather than a change to our data, and the removal pass sends it to console.error or keeps it on the one dog that survives.
