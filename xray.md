...
The wrapper ships in all three, so the expected picture is precise. The P10 tracer appears in every bundle — the public keys are meant to be everywhere. The S10 tracer also appears in every bundle, including the client: a page holds the ciphertext as dead weight it cannot open, since possession of the master key is what separates servers from pages, and encryption rather than delivery is what protects the secret block. What must appear in no bundle at all — client or server, delivered anywhere — is the K10 tracer and every decrypted plaintext value. The client bundle is the sharpest edge: everything in it is public by definition, so a single stray hit there is a leak, full stop.

## The third place secrets live

Beyond the workstation and the bundles there is a third category of location: services in the providers' infrastructure that server code can reach at runtime but that are never part of any delivered bundle. On Cloudflare this is Workers secrets — encrypted variables bound to the worker with `wrangler secret put`, write-only after they're set. On AWS the category has several members — Lambda environment variables, Secrets Manager, Systems Manager Parameter Store — of which we use Lambda environment variables. And on the workstation, the local .env, gitignored and never leaving the machine in plain form.

These three locations — Workers secrets, the Lambda environment, the local .env — are exactly where ACCESS_K10_SECRET is kept, the places it's deliberately stored and managed by hand through each provider's tooling. The deploy machinery also makes working copies in transit — the pipeline section below maps them precisely: the dist echo of .env, the framework's compiled templates, the copies riding to AWS — which is exactly why the audit exists. What xray contributes is telling the deliberate homes and the documented echoes apart from a leak: proving the master key appears nowhere the plan doesn't name.
...
build.js empties and rebuilds `net23/dist/` by hand: it copies `.env` — the master key in plaintext — to `dist/.env`, copies `src/` and `persephone/`, and writes a filtered `dist/serverless.yml` with the BuildRemove-marked lines stripped (those lines exist so serverless-offline can emulate API Gateway locally; deployed, the lambdas use Function URLs). It synthesizes `dist/package.json` by merging net23's and icarus's dependencies and pinning Lambda's platform (linux, arm64, glibc), writes a hoisted-layout `.npmrc` and a blank `pnpm-workspace.yaml` to stop pnpm looking upward, and runs `pnpm install --prod` inside dist. It deletes sharp's musl binaries that pnpm insists on installing beside the glibc ones, copies icarus's .js files in as though they were an installed node module (grid.js excluded — the lambda never imports the grid suite), runs Vercel's node-file-trace from the src entry points and keeps only the node_modules files the trace proves reachable, and finally mutates dist's copy of wrapper.js, flipping `cloud` from false to true.

`serverless deploy` (framework v4) then runs with dist as its working directory. It reads `dist/serverless.yml`, and — v4 reads .env files automatically — resolves every `${env:...}` from `dist/.env` at package time. Resolution happens before template compilation, so the resolved values land as literal strings in the compiled artifacts. Four files appear in `dist/.serverless/`: `cloudformation-template-create-stack.json` (the bootstrap, deployment bucket only), `cloudformation-template-update-stack.json` (the full compiled template), `serverless-state.json` (the serialized resolved service), and `net23.zip` (the code). Observed and exactly as documented: the K10 tracer appears in the update-stack template and the state file — the master key sitting in cleartext JSON on the workstation — and in `dist/.env`, and these three files persist on disk after the deploy finishes. The framework's own documentation warns of precisely this: env-sourced values "can be written into less protected or publicly accessible build logs, CloudFormation templates, et cetera."

The zip is the other half of the story, and it is clean. Observed: 7,537 files, the assembled dist tree packaged verbatim — node_modules, src, persephone, package.json, the lockfile, the .npmrc — with no .env entry (the framework excludes .env files from packages by design) and, streaming the entire zip's contents through the search, zero K10 tracers. The S10 and P10 tracers ride in it once each, in node_modules/icarus/wrapper.js, as they should. The master key reaches the running lambda by a different road entirely: the CloudFormation template carries it to AWS, where it becomes Lambda function configuration — a documented structural separation, code and configuration being distinct objects, with the environment variables never inside the code zip and the zip never containing the environment.

Provider-side, documented: the zip, the compiled template, and the state file all upload to the deployment S3 bucket under `serverless/net23/prod/<timestamp>/` — meaning cleartext copies of the resolved key also persist per-deployment in our own S3. CloudFormation stores the submitted template, retrievable through GetTemplate up to 90 days even after stack deletion. The function configuration holds the environment variables encrypted at rest under an AWS-managed KMS key, readable in cleartext by any principal with GetFunctionConfiguration permission and by the console, and delivered to the runtime as ordinary process environment.
...
The audit sharpened both lists with named residue. On the workstation, the master key legitimately appears in exactly six places the search will hit: the three source homes (the root, site, and net23 `.env` files) and the lambda pipeline's three echoes (`dist/.env` and the two cleartext JSONs under `dist/.serverless/`), all persisting after a deploy. And one decrypted value rides a server bundle by design — the og signing secret in the nitro chunk, the named exception above. A run that finds exactly this picture is clean; a hit anywhere else, or any tracer in the client bundle beyond P10 and today's S10 dead weight, is the alarm. We can see the tracers we should, and cannot find the tracers we shouldn't — and because the values are searchable by design, that assurance costs one command rather than an audit.

## A run of the census, recorded

The full census, run August 23, 2026, against the working tree and the artifacts of the latest builds. Safe to record because the output is only file paths and counts — the tool never prints what surrounds a hit. Lightly edited: log timestamps are trimmed, and the search terms echoed in the output are elided as `(marker)`, because commands name markers assembled and this essay never does.

The K10 census — the master key — matching the named six exactly, three homes and three pipeline echoes:

```
   1  ./.env
   1  ./net23/.env
   1  ./net23/dist/.env
   2  ./net23/dist/.serverless/cloudformation-template-update-stack.json
   3  ./net23/dist/.serverless/serverless-state.json
   1  ./site/.env
9 in 6 files
```

Streaming the lambda zip for the same marker: no matches — the delivered artifact holds no master key.
...
Today one master key does everything. The Serverless Framework reads it from the local .env at deploy time (serverless.yml's environment block) and sets it as a Lambda environment variable, so it rides the deployment's configuration rather than the code zip; Cloudflare holds it as a Workers secret, never in any bundle; and staff workstations hold it in .env. Secure and simple — but a single breach that reaches the one system holding the one key compromises the secrets, because the ciphertext ships in every bundle and the key alone completes the pair.
